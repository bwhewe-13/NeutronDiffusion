...
class Diffusion:
    ...
    def geometry(self):
        ...
        if self.geo == "slab":
            ...
            self.V = np.pi * (
                self.edges[1 : (self.I + 1)] ** 2 - self.edges[0 : self.I] ** 2
            )  # pi(r^2-r^2)
            ...
            self.V = (
                ...
                / 3.0
                * np.pi
                * (self.edges[1 : (self.I + 1)] ** 3 - self.edges[0 : self.I] ** 3)
            )  # 4/3 pi(r^3-r^3)
    ...
    def construct_b(self, phi):
        ...
        for global_x in range(self.G * (self.I + 1)):
            ...
            for group_out in range(self.G):
                ...
                b[global_x] += (
                    self.chi(r)[group_in] * self.fission(r)[group_out] * phi[global_y]
                )
        return b
    ...
    def construct_A_fast(self):
        ...
        slib.construct_A_lambda.argtypes = [
            ctypes.POINTER(full_matrix),
            ctypes.POINTER(cross_section),
    ...
    def construct_A_B(self, RHS=False):
        ...
        for gg in range(self.G):
            ...
            for ii in range(self.I):
                ...
                minus, cell, plus = Diffusion.change_space(
                    self, ii, gg
                )  # move to a given group submatrix

                A[cell, cell] = (
                    2.0
                    / (self.delta * self.V[ii])
                    * (
                        (self.D(r)[gg] * self.D(r + self.delta)[gg])
                        / (self.D(r)[gg] + self.D(r + self.delta)[gg])
                        * self.SA[ii + 1]
                    )
                    + self.removal(r)[gg]
                ...
                for gpr in range(self.G):
                    ...
                    if gpr != gg:  # skip the same group scattering
                        A[cell, prime] = -self.scatter(r)[
                            gg, gpr
                        ]  # scattering diagonal
                    if RHS:
                        B[cell, prime] = (
                            self.chi(r)[gg] * self.fission(r)[gpr]
                        )  # set up the fission diagonal
